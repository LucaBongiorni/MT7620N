...
DelAllSeqNode(ListManage* seqHead)
{
	...
	list_for_each_safe(temp, p, &seqHead->head)
	{
        s = list_entry(temp, SeqListNode, list);  
        if (s->Info.pBuff)
        {
        	ListFree(s->Info.pBuff), s->Info.pBuff = NULL;
        }
        list_del(temp);
		ListFree(s), s = NULL;
    }
...
PickUpBuffFromSeqList(ListManage* seqHead, char** ppBuff)
{
	...
	{
		Debug(I_DEBUG, "Count SeqList Bufflen failed.");
		return -1;
	}
	...
	*ppBuff = (char*)ListMalloc(BuffLen);
	...
	for (i=1; i<=NodeNum; ++i)
	{
		list_for_each_safe(p, pp, &seqHead->head)
		{
			temp = list_entry(p, SeqListNode, list);
	  		if (temp->Info.seqNum == i)
	  		{
				if (temp->Info.pBuff)
		        {
					...
					ListFree(temp->Info.pBuff), temp->Info.pBuff = NULL;
		        }
...
DelOneNodeByTypeID(ListManage* IDHead, int typeID)
{
	...
	list_for_each_safe(p, pID, &IDHead->head)
	{
		temp = list_entry(p, IDListNode, list);
		if (temp->Info.typeID == typeID)
		{
...
PickUpBuffFromIDList(ListManage* IDHead, int typeID, char** buff)
{
	if (NULL == IDHead)
	{
		Debug(I_DEBUG, "Paramter is Error.");
		return -1;
	}
	if ( list_empty(&IDHead->head) )
	{
		Debug(I_DEBUG, "List is empty.");
		return -1;
	}
	...
	list_for_each(p, &IDHead->head)
	{
		...
		if (temp->Info.typeID == typeID)
		{
			...
			{
				Debug(I_DEBUG, "The seq List is empty. typeID=%d", typeID);
				return 0;
			}
			else
			{
...
DelAllNet2SerListNode(ListManage* net2SerHead)
{
    ...
    if (list_empty(&(net2SerHead->head)))
    {
  	...
  	list_for_each_safe(pTemp, pHead, &net2SerHead->head)
  	{
...
int main()
{
	...
	while(1)
	{
		...
		{
			Debug(I_DEBUG, "Add Node To Net2Ser List Failed.");
			return 0;
		}
		if (count == 100)
		{
