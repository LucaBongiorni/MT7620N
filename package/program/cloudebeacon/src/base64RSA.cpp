...
int EVPRSA::openPublicKey(const char *keyfile)  
{
    ...
    if (NULL == bp)  
    {  
        Debug(I_DEBUG, "open_public_key bio file new error!");  
        return -1;  
    }  
    ...
    if (rsa == NULL)  
    {  
        Debug(I_DEBUG, "open_public_key failed to PEM_read_bio_RSAPublicKey!");  
        BIO_free(bp);  
        RSA_free(rsa);   
        return -1;  
    }  

    Debug(I_INFO, "open_public_key success to PEM_read_bio_RSAPublicKey!");  
    m_pubEVPKey = EVP_PKEY_new();  
    if(NULL == m_pubEVPKey)  
    {  
        Debug(I_DEBUG, "open_public_key EVP_PKEY_new failed.");  
        RSA_free(rsa); 
		BIO_free(bp); 
        return -1;  
    }  
...
int EVPRSA::generateKeyFile(const char *pubKeyFile, const char *priKeyFile, int length)
{
    ...
    if (! rsa)  
    {  
        Debug(I_DEBUG, "RSA_generate_key error!");  
        return -1;  
    }  
    ...
    if (NULL == bp)  
    {  
        Debug(I_DEBUG, "generate_key bio file new error!"); 
		RSA_free(rsa); 
        return -1;  
    }  
    ...
    {  
        Debug(I_DEBUG, "BIO_write_filename error!");
		RSA_free(rsa); 
		BIO_free_all(bp);
        return -1;  
    }  
    ...
    {  
        Debug(I_DEBUG, "PEM_write_bio_RSAPublicKey error!");
		RSA_free(rsa); 
		BIO_free_all(bp);
        return -1;  
    } 
	...
	if (NULL == bp)  
    {  
        Debug(I_DEBUG, "generate_key bio file new error2!"); 
		RSA_free(rsa); 
        return -1;  
    } 
...
{   
    ...
    if (NULL == bp)  
    {  
        Debug(I_DEBUG, "open_private_key bio file new error!"); 
		RSA_free(rsa);
        return -1;  
    }  
    ...
    if (rsa == NULL)  
    {  
        Debug(I_DEBUG, "open_private_key failed to PEM_read_bio_RSAPrivateKey!");  
        BIO_free(bp);  
        RSA_free(rsa);  
        return -1;  
    }  
    ...
    if (NULL == m_priEVPKey)  
    {  
        Debug(I_DEBUG, "open_private_key EVP_PKEY_new failed.");  
        RSA_free(rsa);
		BIO_free(bp); 
        return -1;  
    }  
...
{  
    ...
    if (NULL == ctx)  
    {  
        Debug(I_DEBUG, "ras_pubkey_encrypt failed to open ctx.");  
        EVP_PKEY_free(m_pubEVPKey);
		m_pubEVPKey = NULL;
        return -1;  
    }  
    ...
    {  
        Debug(I_DEBUG, "ras_pubkey_encrypt failed to EVP_PKEY_encrypt_init.");  
        EVP_PKEY_free(m_pubEVPKey);  
		m_pubEVPKey = NULL;
        return -1;  
    }  

  	size_t temp = encDataLen;
	int nRet; 
	nRet = EVP_PKEY_encrypt(ctx, encData, &temp, origData, origDataLen);
    if ( nRet <= 0)  
    {  
        Debug(I_DEBUG, "ras_pubkey_encrypt failed to EVP_PKEY_encrypt. nRet=%d", nRet);  
...
{  
    ...
    if (NULL == ctx)  
    {  
		Debug(I_DEBUG, "ras_prikey_decryptfailed to open ctx.\n");  
		EVP_PKEY_free(m_priEVPKey);  
		return -1;  
    }  
    ...
    if (nRet <= 0)  
    {  
        Debug(I_DEBUG, "ras_prikey_decryptfailed to EVP_PKEY_decrypt_init.\n");  
        EVP_PKEY_free(m_priEVPKey);  
        return -1;  
    }  
    ...
    if ( nRet <= 0)  
    {  
        Debug(I_DEBUG, "ras_prikey_decrypt failed to EVP_PKEY_decrypt.");  
        EVP_PKEY_CTX_free(ctx);  
        EVP_PKEY_free(m_priEVPKey);  
        return -1;  
    } 
...
{  
    ...
    if(NULL == fp)  
    {
    	Debug(I_DEBUG, "open file:%s failed.", filePath);
        return -1;  
    }  
...
{  
    ...
    *out = (unsigned char *)malloc(outLen);  
    if (NULL == *out)  
    {   
		Debug(I_DEBUG, "prikey_encrypt:malloc error!");  
        return -1;  
    }  
    memset((void *)*out, 0, outLen);  
  
    Debug(I_INFO, "prikey_encrypt:Begin RSA_private_encrypt ...\n");  
...
{  
    ...
    *out =  (unsigned char *)malloc(outLen);  
    if(NULL == *out)  
    {  
		Debug(I_ERROR, "pubkey_decrypt:malloc error!");  
        return -1;  
    }  
    memset((void *)*out, 0, outLen);  
  
    Debug(I_INFO, "pubkey_decrypt:Begin RSA_public_decrypt ...");  
...
{  
    ...
    *out = (unsigned char *)malloc(outLen);  
    ...
    Debug(I_INFO, "pubkey_encrypt:Begin RSA_public_encrypt ...");  
