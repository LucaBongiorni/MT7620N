...
main(int argc, char** argv)
{
	...
	for(;;)
	{
		...
		// 连接更新服务器
		sockFD = sockConnect->Connect(update.m_webDomain);
		if ( -1 != sockFD)
		{
			// 请求升级信息
			...
			{
				// 需要升级
				if ( 0 != update.updateSystem(kind, filePathA, filePathB, md5, ver) )
				{
		...
		else
		{
			...
			if (sockConnect) delete sockConnect, sockConnect = NULL;
