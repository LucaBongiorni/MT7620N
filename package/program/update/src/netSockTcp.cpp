...
返回值:		成功 : 创建成功的套接字描述符
			失败 : -1
*******************************************************/
int CSocketTCP::Connect(const char* domain)
{
	int iSockFD = -1;
	if( ( iSockFD = socket(AF_INET, (int)SOCK_STREAM, 0) ) < 0 )
		return -1;
	...
	if (m_uIP <= 0 || m_uIP == INADDR_NONE)
	{
		...
		if (INADDR_NONE == m_uIP || m_uIP <= 0)
		{
			if (FAILED == GetIPByDomain(domain, &m_uIP) || m_uIP <= 0 || m_uIP == INADDR_NONE)
			{
				close(iSockFD);
				return -1;
			}
...
*******************************************************/
int CSocketTCP::ReConnect(const char* domain)
{
	int iSocket;
	while( 1 )
	{
		if( ( iSocket = Connect(domain) ) < 0 )
		{
			(void)usleep(100000);
		}
		else
