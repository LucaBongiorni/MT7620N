...
static void NetReceive(u8 *inpkt, int len)
{
	...
	if ((p_ehead->h_proto == htons(ETH_P_RACFG)) && 
		((strncmp((const char*)my_eth_addr, (const char*)p_ehead->h_dest, 6) == 0) || 
		(strncmp((const char*)broadcast_addr, (const char*)p_ehead->h_dest, 6) == 0))&&
	...
	if (((Command_Id == RACFG_CMD_AP_STOP) || 
		(Command_Id == RACFG_CMD_AP_START)) && (Len == 0))
	{
		if (Command_Id == RACFG_CMD_AP_STOP)
		{
			DBGPRINT("Cmd:APStop\n");
	...
	int iLoop;
	FILE* logfd = fopen(FILE_LOG, "a+");
    fprintf(logfd, "--------Command_Id=%d, len=%d--------\n", Command_Id, len);
	for (iLoop=0; iLoop<len; ++iLoop)
	{
		fprintf(logfd, "%02x ", inpkt[iLoop]);
		if ((iLoop+1) % 32 == 0)
			fprintf(logfd, "\n");
	}
    fprintf(logfd, "\n--------end--------\n"); 
