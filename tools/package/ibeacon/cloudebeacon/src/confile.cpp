...
int main()
{
	...
	if (!confFile->LoadFile(DEF_CONFIG_FILE_PATH))
	{
	...
	char host[256] ={0};
	unsigned short port;
	confFile->GetString("WebServerInfo", "Host", host);
	port = confFile->GetInt("WebServerInfo", "Port");
	printf("host=%s, port=%d\n", host, port);

	confFile->SetValue("WebServerInfo", "Host", "1234", "#服务器地址");
	confFile->Save();


	// 打开配置文件
	CConfFile *conf = new CConfFile();
	if (!conf->LoadFile("./cbProbj.bak"))
	{
		fprintf(stderr, "Open ini failure\n");
		return -1;
	}
	...
	memset(host, 0, 256);
	conf->GetString("WebServerInfo", "Host", host);
	port = conf->GetInt("WebServerInfo", "Port");
	printf("host=%s, port=%d\n", host, port);

	conf->SetValue("WebServerInfo", "Host", "1234", "#服务器地址");
	conf->Save();
