	...
	: m_nRecvTimeO(10000), m_nSendTimeO(10000)
{
	...
	if (IP == INADDR_NONE)
	{
		IbeaconConfig::GetIPByDomain(uIP, &IP);
	}
	m_uIP = IP;
	//m_MemSize = 1500;
...
int 
CSocketTCP::Connect(unsigned int ulTimeout)
{
    int nRet;
    int error = 0;
    socklen_t len;
    ...
    if (SockFd != -1)
    {
        ...
        if (nRet == -1)
        {
            ...
            {
                write_error_log("0 connect err:%s", strerror(errno));
                Close(SockFd);
                return -1;
            }
            struct timeval tm;
            ...
            while(1)
            {
                ...
                case -1:
                    ...
                    write_error_log("1 connect err:%s", strerror(errno));
                    Close(SockFd);
                    return -1;
                case 0:
                    write_error_log("2 connect: timeout");
                    Close(SockFd);
                    return -1;
                default:
                    len = sizeof(int);
                    ...
                    write_error_log("5 connect err:%s", strerror(errno));
                    Close(SockFd);
                    return -1;
                }
            }
...
Read(int iSockFD, char * cData, int iLen, int iTimeOut )
{
	...
	//write_normal_log("AAAAAAAAAAAAAAAAAAAAAA");

	struct timeval tv;
	tv.tv_sec = iTimeOut / 1000;	// 设置读超时
	tv.tv_usec = iTimeOut % 1000;
	(void)setsockopt(iSockFD, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv) );
	//write_normal_log("BBBBBBBBBBBBBBBBBBBBBBB");
	
	ptr   = cData;
	nleft = iLen;
	while ( nleft > 0 )
	{
		//write_normal_log("CCCCCCCCCCCCCCCCCCCCCC");
		if ( (nread = read(iSockFD, ptr, nleft) ) < 0 )
		{
			//write_normal_log("DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD EAGAIN=%d, errno=%d, nread=%d, %m", EAGAIN, errno, nread);
			if (EINTR == errno)  // || EAGAIN == errno
...
int Recv(int nTcpSocket, char *pBuf, int nLen, unsigned int ulTimeout)
{
    ...
    if (ret > 0)
    {
		while ( nleft > 0 )
		{
			...
			{
				if (errno == EINTR || errno == EWOULDBLOCK || errno == EAGAIN)  // || EAGAIN == errno
					nread = 0;				// 被中断程序中断，重新调用recv()
				else 
				{
		...
		//write_normal_log("nLen-nleft=%d", nLen-nleft);
		return (nLen-nleft);				// return >= 0
    }
    else
    {
    	write_normal_log("ret=%d, errno=%d", ret, errno);
        return ret;
    }
}
